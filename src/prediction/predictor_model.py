...
class TSAnnotator:
    ...
    def build_NNet_model(self) -> Sequential:
        """Build a new ANN annotator."""
        model = Sequential(name="ANN_Timeseries_Annotator")
    
    # Assuming input shape is (encode_len, feat_dim)
        # model.add(Permute((2, 1), input_shape=(self.encode_len, len(self.data_schema.features))))
        
        model.add(layers.Dense(1000))
        model.add(layers.Dense(200, activation=self.activation))
        model.add(layers.Dense(100))
        model.add(layers.Dense(100, activation=self.activation))
    ...
    def save(self, model_dir_path: str) -> None:
        ...
        if not self._is_trained:
            raise NotFittedError("Model is not fitted yet.")
        self.net.save(os.path.join(model_dir_path, (PREDICTOR_FILE_NAME +".keras")))
        self.net = None
    ...
    def load(cls, model_dir_path: str) -> "TSAnnotator":
        ...
        model.net = keras.saving.load_model(os.path.join(model_dir_path, (PREDICTOR_FILE_NAME +".keras")))
        return model
